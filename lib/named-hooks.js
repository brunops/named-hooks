...
NamedHooks.prototype.invokeSync = function (hookName, identifier, data) {
  ...
  debug('#invokeSync: invocation of hooks with base hookName "%s" and identifier "%s" resulted with: %j', hookName, identifier, result);
  return result;
};
...
NamedHooks.prototype.invoke = function (hookName, identifier, data) {
  ...
  // wrap it in a function if method is called with only two parameters
  // so it's possible to use it easily in a promise chain without
  // crazy bindings
  // i.e.:
  //    q(5)
  ...
  if (typeof data === 'undefined') {
    return function (prevData) {
      debug('#invoke: called with no data to pass through hooks. Returning a function that accepts the data as an argument when called');
      return _getPromiseChain(existingFunctions, prevData);
    };
  }

  result = _getPromiseChain(existingFunctions, data);
  debug('#invoke: invocation of hooks with base hookName "%s" and identifier "%s" resulted with: %j', hookName, identifier, result);
  return result;
};
